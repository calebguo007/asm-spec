#!/usr/bin/env python3
"""ASM 真实 A/B 测试 — 通过 Knot API 调用真实 LLM 服务。
...
def print_real_report(analysis: dict[str, Any]) -> None:
    ...
    print("\n" + "=" * 78)
    print("  ASM 真实 A/B 测试报告")
    print("  (Real API Calls — NOT Simulated)")
    print("=" * 78)
    print(f"\n  时间: {info['timestamp']}")
    print(f"  总 API 调用数: {info['total_api_calls']}")
    print(f"  测试 prompt 数: {info['prompts_used']}")
    print(f"  测试类型: 真实 API 调用 🔴")
    ...
    metrics = [
        ...
        ("实际延迟 (s)", "actual_latency_mean", "actual_latency_std"),
    ...
    print(f"\n  统计显著性 (Welch's t-test, α=0.05):")
    print(sep)
    metric_labels = {
        "topsis_score": "TOPSIS 得分",
        "actual_latency_s": "实际延迟",
        "quality_score": "响应质量",
    }
    ...
    for g_name in ["A_ASM", "B_Random", "C_Expensive"]:
        td = trust[g_name]
        print(f"  {g_name:<16} 平均偏差: {td['latency_delta_mean']:.2%} ± {td['latency_delta_std']:.2%}")
    ...
    print(f"  3. TOPSIS A vs B 显著性: p={t_tests['topsis_score']['A_vs_B']['p']:.6f} {'✅ 显著' if topsis_sig else '⚠️ 不显著'}")
    print()
...
def main():
    parser = argparse.ArgumentParser(description="ASM 真实 A/B 测试 — 通过 Knot API 调用真实 LLM")
    ...
    if not manifests:
        ...
        sys.exit(1)
    print(f"✅ 加载了 {len(manifests)} 个 ASM manifests")
    ...
    print(f"✅ 收集了 {len(results)} 条真实测试记录")
