#!/usr/bin/env node
/**
 * asm-lint — MCP Server Quality Detection CLI
 *
 * Scan any MCP Server and generate a quality report:
 *   - Tool definition completeness
 *   - Schema validity
 *   - Latency probing
 *   - x-asm metadata check
 *   - Overall scoring
 *
 * Usage:
 *   npx asm-lint <mcp-server-command>
 *   npx asm-lint --json <mcp-server-command>
 *   npx asm-lint --init  (generate x-asm metadata template)
 */
...
// ── Color output (lightweight, no chalk dependency) ──────────────────────
const c = {
...
function checkToolDefinitions(tools: ToolDef[]): Check {
    ...
    const wellDefined = tools.filter((t: ToolDef) => t.description && t.description.length > 10);
    ...
    const missing = tools.filter((t: ToolDef) => !t.description || t.description.length <= 10);
    return {
        name: "Tool Definitions",
        status: "warn",
        message: `${tools.length} tools, ${missing.length} missing/short descriptions: ${missing.map((t: ToolDef) => t.name).join(", ")}`,
...
function checkInputSchemas(tools: ToolDef[]): Check {
    ...
    const withSchema = tools.filter((t: ToolDef) => t.inputSchema && typeof t.inputSchema === "object");
    const withProperties = withSchema.filter((t: ToolDef) => {
...
function checkDescriptionQuality(tools: ToolDef[]): Check {
    ...
    for (const t of tools) {
        ...
        if (desc.length > 30)
            ...
            issues.push(t.name);
        // Check if description includes usage examples or parameter docs
        if (desc.includes("e.g.") || desc.includes("example") || desc.includes("returns"))
            score += 1;
    }
...
function checkNamingConventions(tools: ToolDef[]): Check {
    ...
    const wellNamed = tools.filter((t: ToolDef) => snakeCase.test(t.name));
    ...
    const bad = tools.filter((t: ToolDef) => !snakeCase.test(t.name)).map((t: ToolDef) => t.name);
...
function checkXAsmMetadata(tools: ToolDef[]): Check {
    // Check if tools include x-asm extension metadata
    const withXAsm = tools.filter((t: ToolDef) => {
        const schema = t.inputSchema;
        if (!schema)
            return false;
...
function checkErrorHandling(tools: ToolDef[]): Check {
    // Check if tool descriptions mention error handling
    const mentionsErrors = tools.filter((t: ToolDef) => {
...
function checkSecurityPatterns(tools: ToolDef[]): Check {
    // Check for potentially risky tools (e.g., filesystem access, command execution)
    const riskyPatterns = ["exec", "shell", "command", "file_system", "write_file", "delete", "rm ", "sudo"];
    const riskyTools = tools.filter((t: ToolDef) => {
    ...
    return {
        ...
        status: "warn",
        message: `${riskyTools.length} potentially risky tools: ${riskyTools.map((t: ToolDef) => t.name).join(", ")}. Consider adding confirmation prompts.`,
        points: 5,
        maxPoints: 10,
    };
}
function checkIdempotency(tools: ToolDef[]): Check {
    // Check if tool descriptions indicate idempotency
    const readOnly = tools.filter((t: ToolDef) => {
        const desc = (t.description || "").toLowerCase();
        const name = t.name.toLowerCase();
...
// ── Main flow ───────────────────────────────────────────────
async function lint(command: string, args: string[]) {
    ...
    for (const t of tools) {
        const paramCount = t.inputSchema && t.inputSchema.properties
...
// ── --init command: generate x-asm metadata template ──────────────────
...
`);
    ...
    const cmdArgs = args.filter((a: string) => a !== "--json");
    if (cmdArgs.length === 0) {
